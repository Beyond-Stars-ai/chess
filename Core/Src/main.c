...
/* Private typedef -----------------------------------------------------------*/
/* USER CODE BEGIN PTD */
uint8_t sensors_Text[9]; // 传感器数据
uint8_t board_End[9];    // 传感器数据
...
static void MX_GPIO_Init(void)
{
    ...
    /*Configure GPIO pins : sensor_1_Pin sensor_2_Pin sensor_3_Pin sensor_4_Pin
                             ...
                             KEY_4_Pin */
    GPIO_InitStruct.Pin = sensor_1_Pin | sensor_2_Pin | sensor_3_Pin | sensor_4_Pin | sensor_5_Pin | sensor_6_Pin | sensor_7_Pin | KEY_5_Pin | KEY_4_Pin;
    ...
    /*Configure GPIO pins : sensor_8_Pin sensor_9_Pin KEY_3_Pin KEY_2_Pin
                             KEY_1_Pin */
    GPIO_InitStruct.Pin = sensor_8_Pin | sensor_9_Pin | KEY_3_Pin | KEY_2_Pin | KEY_1_Pin;
    GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
    GPIO_InitStruct.Pull = GPIO_PULLUP;
...
void StartAILogic(void *argument)
{
    ...
    for (;;)
    {
        ...
        if ((flags & FLAG_AI_START) && current_state == STATE_AI_THINK && chessgame.game_mode == GAME_MODE_PLAY)
        {
        ...
        if ((flags & FLAG_AI_START) && current_state == STATE_AI_THINK && chessgame.game_mode == GAME_MODE_TEXT)
        {
            int move = AI_GetBestMove(chessgame.board, chessgame.ai_color);
...
void StartDebugTask(void *argument)
{
    ...
    for (;;)
    {
        ...
        for (int i = 0; i < 9; i++)
        {
            len = sprintf(debug_buf, "s%d: %d ", i + 1, sensors_Text[i]);
            HAL_UART_Transmit(&huart1, (uint8_t *)debug_buf, len, 100);
        }
        HAL_UART_Transmit(&huart1, (uint8_t *)"\r\n", 2, 100);
