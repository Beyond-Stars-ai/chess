...
static void MX_GPIO_Init(void)
{
    ...
    GPIO_InitStruct.Pin = sensor_1_Pin | sensor_2_Pin | sensor_3_Pin | sensor_4_Pin | sensor_5_Pin | sensor_6_Pin | sensor_7_Pin;
...
void StartReadKEY(void *argument)
{
  ...
  for (;;)
  {
    cur_state = (HAL_GPIO_ReadPin(KEY_GPIO_Port, KEY_Pin) == GPIO_PIN_RESET) ? 0 : 1;
